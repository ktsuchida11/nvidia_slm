...
# ループ9(finqa): GRPO・S6評価は同一のFINQA_SYSを使う(学習と評価の整合 —
# ループ8で訓練だけ思考モードONの非対称が空振りの一因になった教訓)。
# 「答え:」形式は報酬/評価の数値抽出を安定させるため。形式習得自体もGRPOの学習信号。
# SFT(汎用金融QA)は形式を強制しないFINQA_CHAT_SYSを使う(回答データが形式を持たないため)
FINQA_SYS = ("あなたは日本の金融アシスタント。質問に日本語で簡潔に答える。"
             "計算が必要な場合は途中式を短く示す。"
             "最後の行を「答え: <数値><単位>」の形式で終えること。")
FINQA_CHAT_SYS = "あなたは日本の金融アシスタント。質問に日本語で正確かつ簡潔に答える。"
