...
output "connect_gpu" {
  description = "GPUノードへのSSM接続コマンド"
  value = length(module.gpu_host.instance_ids) > 0 ? format(
    "aws ssm start-session --target %s%s --region %s",
    module.gpu_host.instance_ids[0],
    var.enable_bastion ? format(" --document-name %s", module.bastion[0].session_document_name) : "",
    var.region,
