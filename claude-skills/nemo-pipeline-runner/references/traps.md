...
追記したら**配布先へ再コピーする**（`cp -r claude-skills/nemo-pipeline-runner .claude/skills/`）。
...
| **スキルが「消化済みのコンポーネントを未消化」と答える**（内容は破綻していないので気づけない） | 書き戻し先は `claude-skills/` だが、エージェントが読むのは `cp -r` した `.claude/skills/` の配布物。loop12〜15 の書き戻しを4回やって、コピーを1回も再実行していなかった＝**稼働中のスキルが loop11 時点で凍結** | 総括の最後に**必ず `cp -r claude-skills/nemo-pipeline-runner .claude/skills/`** を実行する（loop-XX-report を書いたら配布まででワンセット）。SKILL.md 冒頭の「反映範囲」行を最新 loop 番号と突き合わせれば古さは即判定できる |
